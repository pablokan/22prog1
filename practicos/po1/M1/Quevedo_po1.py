...
for i in range(len(deudores)):
    deudor = deudores[i]
    posComa = deudor.split(",")
    plata = posComa[4]
    dinero.append(plata)
print(dinero)
for x in range(len(dinero)):
    deuda = dinero[x]
    sinSigno = deuda.split("$")
 #   deudaIndividual = sinSigno[x]
 #   for i in range(len(sinSigno)):
 #       c += deudaIndividual[i]
...
print("La cantidad de mujeres que tengo son:", chicas)
